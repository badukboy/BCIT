/*
	COMP2510 ASSIGNMENT 1 - STUDENT SCORES
...
/* 
	...
	const char * choices[] - Array containing the available menu choices.
	const char * prompr - Prompt keyword to indicate user input field.
*/
int menu(const char * choices[], const char * prompt)
{
	size_t i;
...
/* 
	Allows the user enter student names to the text file.
	
	FILE * fileInput - The input file.
	FILE * standardInput - Standard in.
	const char * prompt - Prompt keyword to indicate user input field.
...
int enterData(FILE * fileInput, FILE * standardInput, const char * prompt)
{
	...
	while(fgets(inputLine, LINESIZE, standardInput) != NULL)
	{
		...
		if((validation = sscanf(inputLine, " %[a-zA-Z-] %[a-zA-Z-] %d ", 
				firstName, lastName, &junk)) == 3)
		{
		...
		if((validation = sscanf(inputLine, " %[a-zA-Z-] , %[a-zA-Z-] ", 
		...
		if((validation = sscanf(inputLine, " %[a-zA-Z-] %[a-zA-Z-] ", firstName, 
				lastName)) == 2)
		{		
			printf("Please enter a score between 0 and 100 inclusive for this student:\n");
			printf("%s", prompt);
...
/*
	Simple function that displays available student data recorded so far.
...
int displayData(FILE * fileInput)
{
	...
	{
		if((validation = sscanf(inputLine, "%s %s %d", firstName, lastName, 
				&score)) == 3)
		{
